...
#let vbar(color) = text(fill: color)[#h(1em)|#h(1em)]
...
#let header(color: str, data: list) = {
  grid(
    ...
    grid(
      ...
      [],
      text(size: 11pt)[
        #icon(color, fa-github()) #link("github.com/" + data.github, data.github) #vbar(color)
        #icon(color, fa-linkedin()) #link("linkedin.com/" + data.linkedin, data.linkedin) #vbar(color)
